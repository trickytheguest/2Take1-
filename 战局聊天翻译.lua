local ScriptName = "战局聊天翻译"
local Version = "1.7.3"

if ProddysTranslator then
	menu.notify("脚本已加载", ScriptName, 10, 0xFF0000FF)
	return
end

if not menu.is_trusted_mode_enabled(eTrustedFlags.LUA_TRUST_HTTP) then
	menu.notify("脚本需要HTTP可信标志", ScriptName, 10, 0xFF0000FF)
end
...
local function TranslateChat(event)
	...
	if not menu.is_trusted_mode_enabled(eTrustedFlags.LUA_TRUST_HTTP) then
		notify("聊天消息未翻译，因为未启用 HTTP 信任模式。", nil, nil, 0xFF0000FF)
		return
	end
	if (Settings.TranslateSelf or pid ~= player.player_id()) and player.is_player_valid(pid) then
		...
		if sourceLang ~= Settings.TargetLang and not ExcludedLanguages[sourceLang] then
			notify(translation, name .. ": 翻译自 " .. (LangLookupByKey[sourceLang] or sourceLang),nil,0xFFFFFF00 )
		end
	end
end
...
local EnableTranslationFeat = menu.add_feature("启用翻译", "toggle", ParentId, function(f)
...
local EngineParentId = menu.add_feature("翻译引擎", "parent", ParentId).id

local EngineFeat = menu.add_feature("翻译引擎", "autoaction_value_str", EngineParentId, function(f)
...
local DeepLFreeKeyFeat = menu.add_feature("输入 DeepL 自由密钥", "action", EngineParentId, function(f)
	local r, s
	repeat
...
local DeepLProKeyFeat = menu.add_feature("设置 DeepL Pro 密钥", "action", EngineParentId, function(f)
	local r, s
	repeat
		r, s = input.get("Set DeepL Pro Key", Settings.DeepLProKey or "", 39, 0)
		if r == 2 then return HANDLER_POP end
		system.wait(0)
	until r == 0
...
menu.add_feature("检查 DeepL 免费密钥的使用情况", "action", EngineParentId, function(f)
	if not Settings.DeepLFreeKey or Settings.DeepLFreeKey:len() == 0 then
...
menu.add_feature("检查 DeepL Pro 密钥使用情况", "action", EngineParentId, function(f)
	if not Settings.DeepLProKey or Settings.DeepLProKey:len() == 0 then
...
local TargetLangFeat = menu.add_feature("目标语言", "autoaction_value_str", ParentId, function(f)
...
local TranslateSelfFeat = menu.add_feature("翻译自己", "toggle", ParentId, function(f)
...
local ExcludedParentId = menu.add_feature("排除的语言", "parent", ParentId).id
local SendOptionsId = menu.add_feature("发送消息选项", "parent", ParentId).id
...
menu.add_feature("发送翻译信息", "action_value_str", ParentId, function(f)
	local TargetLang = LangLookupByName[LangKeys[f.value + 1]]
	
	local r, s
	repeat
		r, s = input.get("输入要翻译的文本", "", 255, 0)
		if r == 2 then return HANDLER_POP end
		system.wait(0)
	until r == 0
...
for i=1,#Languages do
	...
	local sendFeat = menu.add_feature("发送 " .. language.Name .. " 信息", "action", ParentId, SendLanguage)
